...
PROMPT_DIRTRIM=10 # bash4 and above
...
fg_color() {
	...
	lblue)	    echo 38\;5\;24;;
        darkgray)   echo 38\;5\;236;;
	gray)	    echo 38\;5\;245;;
	myellow)    echo 38\;5\;228;;
    esac
}
...
bg_color() {
	...
	lblue)      echo 48\;5\;24;;
	darkgray)   echo 48\;5\;236;;
	gray)	    echo 48\;5\;245;;
	yellow)     echo 48\;5\;228;;
    esac;
}
...
prompt_context() {
    ...
    if [[ $user != $DEFAULT_USER || -n $SSH_CLIENT ]]; then
        prompt_segment lblue myellow "$user@\h"
    fi
}
...
git_status_dirty() {
    ...
    [[ -n $dirty ]] && echo -e " \e[38;5;124m+$(git status -s | wc -l)\e[1m"
}
...
prompt_dir() {
    prompt_segment darkgray gray '\w'
}
...
prompt_status() {
    ...
    [[ $(jobs -l | wc -l) -gt 1 ]] && symbols+="$(ansi_single $(fg_color cyan))⚙"
